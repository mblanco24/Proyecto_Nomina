...
async function cargarTablaBiometrico() {
    try {
        const response = await fetch(`${API_BASE_URL}/pagos/biometrico/ver`);
        ...
        Object.values(agrupado).forEach(usuario => {
            ...
            tr.innerHTML = `
                ...
                ${diasSemana.map(d => {
                    ...
                    return `
                        <td class="text-center ${entrada === 'No registrado' ? 'text-muted' : ''}">${entrada}</td>
...
// Ejecutar al cargar la página o cuando desees mostrar los datos
